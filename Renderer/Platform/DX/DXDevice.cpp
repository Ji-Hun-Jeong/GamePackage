...
namespace Graphics::DX
{
	...
	TDeviceInitData CDXDevice::CreateContextAndSwapChain()
	{
		...
		HRESULT hr = ::D3D11CreateDeviceAndSwapChain(nullptr, driverType, 0
			...
			, RawSwapChain.GetAddressOf(), Device.GetAddressOf()
		...
		hr = RawSwapChain->GetBuffer(0, IID_PPV_ARGS(WindowTextureBuffer.GetAddressOf()));
		if (FAILED(hr))	assert(0);

		size_t WindowBufferHandle = DXResourceStorage.InsertResource(WindowTextureBuffer);
		auto WindowTexture = std::make_unique<CTexture2D>(WindowBufferHandle, std::bind(&CDXDevice::ReleaseResource, this, std::placeholders::_1));

		Context = std::make_unique<CDXContext>(RawContext, DXResourceStorage);
		SwapChain = std::make_unique<CDXSwapChain>(std::move(WindowTexture), RawSwapChain, NumOfMultiSamplingLevel);
		return TDeviceInitData{ Context.get() , SwapChain.get() };
	}
