...
CGame::CGame(UINT InScreenWidth, UINT InScreenHeight)
    ...
    , World()
    , Device(std::make_unique<Graphics::DX::CDXDevice>(Window.GetWindowHandle()))
    , Context(nullptr)
    , SwapChain(nullptr)
{
    Graphics::TDeviceInitData InitData = Device->CreateContextAndSwapChain();

    Context = InitData.Context;
    SwapChain = InitData.SwapChain;
